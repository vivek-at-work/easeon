...
def get_device(ticket):
    return {
        'id': ticket.device.identifier
    }


def get_notes(ticket):
    return [{"type": "CUSTOMER_INTAKE_NOTES",
...
def get_repair_flags(ticket):
    return {"requestReviewByApple": True,
            "markComplete": True}
...
def get_customer_info(ticket):
    ...
    return {"firstName": customer.first_name,
