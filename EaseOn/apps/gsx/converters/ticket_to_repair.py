...
class TicketToRepairConverter:
            ...
            "address": [
                {
                    ...
                    "line2": customer.address_line_2[0:59]
                    if customer.address_line_2 is not None
                    else None,
    ...
    def get_device(self):
        ...
        return {"id": device.serial_number}
    ...
    def get_technician_note(self):
        if hasattr(self.ticket, "delivery") and self.ticket.delivery:
            return {"type": "TECHNICIAN", "content": self.ticket.delivery.action_taken}
        return {"type": "TECHNICIAN", "content": None}
                ...
                results.append(
                    {
                        ...
                        "device": {"id": inventory_item.serial_number},
                    }
                )
            return results
    ...
    def convert(self):
        self.data = {
            "notes": [self.get_customer_intake_note()],
        ...
        if self.ticket.device.gsx_repair_type in ["MINS", "MINC"]:
            self.data["boxRequired"] = self.ticket.box_required
...
def get_convertor_class(gsx_repair_type):
    d = {"SVNR": SVNRTicketToRepairConverter}
