...
class GSXRequest:
    ...
    def _process_errors(self,message):
        ...
        if 'errorId' in message and 'errors' in message:
            logging.error('Error Response %s received from GSX for url %s',message,self.url)
    ...
    def _send_request(self,method,url,payload_string=None):
        ...
        if is_unauthorized and self.can_handle_token_timout:
    ...
    def post(self, **kwargs):
        ...
        if 'payload' in kwargs.keys():
            data = kwargs['payload']
        message , error_codes , is_unauthorized = self._send_request('POST',self.url,json.dumps(data))
        return message
