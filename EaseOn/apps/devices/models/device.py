...
class Device(BaseModel):
    ...
    def fetch_diagnosis_console_url(self, eventNumber, gsx_username, authtoken):
        ...
        req = GSXRequest("diagnostics", "customer-report-url", gsx_username, authtoken)
    ...
    def run_diagnosis_suite(self, suiteId, gsx_username, authtoken):
        ...
        device = {"id": self.identifier}
        req = GSXRequest("diagnostics", "initiate-test", gsx_username, authtoken)
    ...
    def get_warranty(self, gsx_username, authtoken, **kwargs):
        ...
        req = GSXRequest(
            "repair", "product/details?activationDetails=true", gsx_username, authtoken
        )
        device = {"id": self.identifier}
    ...
    def get_component_issue(self, gsx_username, authtoken, **kwargs):
        req = GSXRequest("repair", "product/componentissue", gsx_username, authtoken)
        device = {"id": self.identifier}
        response = req.post(device=device)
        return response
